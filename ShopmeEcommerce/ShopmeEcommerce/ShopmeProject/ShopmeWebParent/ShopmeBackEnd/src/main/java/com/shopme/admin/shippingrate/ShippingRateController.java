...
public class ShippingRateController {
	...
	public String listByPage(
			@PagingAndShortingParam(listName = "shippingRates",
	...
	public String updateCODSurport(@PathVariable("id") Integer id,
			@PathVariable(name = "supported") Boolean supported,
			Model model, RedirectAttributes ra) {
		
				try {
					service.updateSODSurpport(id, supported);
					ra.addFlashAttribute("message", "COD surpport for shipping rate ID " + id + " has been updated.");
				}catch (ShippingRateNotFoundException e) {
					ra.addFlashAttribute("message", e.getMessage());
				}
				return defaultRedirectURL;		
